...
//! directory -- wherever `Cargo.toml` is. However, if you are using a workspace
//! or have a more complicated build setup, this build script becomes required.
//! Additionally, by requesting that Cargo re-run the build script whenever
//! `memory.x` is changed, updating `memory.x` ensures a rebuild of
//! the application with the new link settings.
...
fn main() {
    ...
    // script is only re-run when `memory.x` is changed.
    println!("cargo::rerun-if-changed=memory.x");

    // Tell the linker to use the cortex-m-rt linker script, which pulls in our
    // memory.x
